...
**GOTCHA NSIS (crítico, medido):** `nsExec::ExecToLog/ExecToStack` NO pasa por cmd.exe, por lo que la redirección `mysql ... < script.sql` no funciona (mysql recibe `<` como argumento literal → exit 1 y no ejecuta nada). Todo `nsExec` con `<` debe envolverse: `nsExec::ExecToLog 'cmd /c ""$MySQLBinPath\mysql.exe" -u root < "$TEMP\x.sql""'`. Esto era un bug latente del instalador viejo (los métodos 0/2/4 de seteo de password nunca andaban; salvaba mysqladmin del método 1). **Extensión 2026-09-06:** lo mismo vale para `2>&1` — `nsExec::ExecToStack '"$MySQLBinPath\mysql.exe" ... -e "SELECT 1;" 2>&1'` hace que mysql tome `2>&1` como **nombre de BD** (`ERROR 1049 Unknown database '2>&1'`, exit 1 siempre). Todas las verificaciones con `2>&1` en `nsExec` (VerifyPassword64, probe reparsoft_app, CheckDBExists) fallaban siempre aunque las credenciales estuvieran bien → el instalador concluía "reparsoft_app no autentica" y no creaba BDs. Fix: envolver en `cmd /c ""$MySQLBinPath\mysql.exe" ... 2>&1"'`. Los `2>&1`/`2>nul` dentro de `.bat` generados (FileWrite) sí son válidos (corren en cmd real).
