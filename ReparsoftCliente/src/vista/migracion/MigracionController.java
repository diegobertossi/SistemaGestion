 ...
 * FIX: Se eliminó INSERT IGNORE para tablas con PK manual. Ahora se hace SELECT
 * EXISTS antes de intentar cualquier INSERT. Si ya existe → mapear al ID de
 * MySQL y continuar. Si no existe → INSERT con ID explícito de Access. Si el ID
 * colisiona → obtener MAX(pk)+1 y usar ese nuevo ID. El nuevo ID se propaga a
 * todas las FK relacionadas (staging).
 *
 * BUG 2: "Field 'idCliente' doesn't have a default value" (cliente, sucursal,
 * equipos, remitos). Las PK de MySQL NO tienen AUTO_INCREMENT. → FIX: No se usa
 * INSERT sin ID. Se calcula MAX(pk)+1 cuando hay colisión.
 *
 * BUG 3: "Duplicate entry '0' for key remitos.PRIMARY" en cadena. → FIX: Para
 * remitos, el criterio de coincidencia es la PK directa (idRemito ==
 * CodigoRemito de Access) porque hay correspondencia 1:1. El registro
 * idRemito=0 de MySQL ya existe → se mapea, no se inserta.
 *
 * BUG 4: "fk_equipos_sucursal" — equipos con IdSucursal inexistente en MySQL. →
 * FIX: Antes de insertar un equipo, se verifica si su IdSucursal existe en la
 * tabla sucursal del destino. Si no existe → se pone NULL.
 *
 * BUG 5: "fk_reemplazos_rep" — reemplazos con ELS sin reparación en destino. →
 * FIX: Se verifica existencia del ELS en reparaciones del destino antes de
 * insertar cada reemplazo.
 *
 * REGLA GENERAL: MySQL manda. Se busca por campo natural primero. Si existe en
 * MySQL → usar su ID, ajustar FK en staging. Si no existe → insertar con ID de
 * Access si no colisiona, sino MAX+1. El mapa resultante garantiza que las FK
 * (idEquipo en reparaciones, idCliente en sucursal, etc.) siempre apunten a IDs
 * válidos de MySQL.
 */
public class MigracionController {
	...
	private void extSucursales(Connection acc, Connection stag) throws SQLException {
		...
		try (Statement st = acc.createStatement();
				...
				PreparedStatement ps = stag.prepareStatement("INSERT IGNORE INTO sucursal "
			...
			while (rs.next()) {
				...
				// FIX: NombreSucursal NULL o vacío → "" en lugar de NULL
	...
	// Si hay colisión de IDs → MAX(pk)+1. Sin INSERT IGNORE en tablas sin
	// AUTO_INCREMENT.
	// ════════════════════════════════════════════════════════════════════════

	// ── Utilidad: obtener siguiente ID disponible (MAX+1) para tablas sin
	// AUTO_INCREMENT
	...
	private Map<Integer, Integer> mergeUbicacionRemitos(Connection stag, Connection dest) throws SQLException {
		...
		try (Statement st = stag.createStatement();
			...
			while (rs.next()) {
				...
				buscPS.setString(1, ubic);
				buscPS.setObject(2, cod);
	...
	// Se verifica por PK primero. Si ya existe → mapear. Si no → insertar con mismo
	// ID.
		...
		try (Statement st = stag.createStatement();
			...
			while (rs.next()) {
				...
				if (existePK(dest, "cliente", "idCliente", idAcc)) {
					...
					// Propagar cambio de ID en staging (sucursal y equipos que referencian este
					// idCliente)
				...
				try {
					...
					err_++;
					log("ERROR", "  mergeCliente id=" + idAcc + ": " + ex.getMessage());
				}
			}
		}
...
// Búsqueda: trata NULL y '' como el mismo valor (cadena vacía)
		String buscar = "SELECT IdSucursal FROM sucursal WHERE COALESCE(NULLIF(NombreSucursal, ''), '') = ? AND idCliente <=> ? LIMIT 1";

		String ins = "INSERT INTO sucursal (IdSucursal, NombreSucursal, idCliente, DomicilioSucursal, ContactoSucursal, TelefonoSucursal, CorreoElectronico) VALUES(?,?,?,?,?,?,?)";

		int ins_ = 0, omit_ = 0, err_ = 0;

		try (Statement st = stag.createStatement();
				ResultSet rs = st.executeQuery("SELECT IdSucursal, NombreSucursal, idCliente, DomicilioSucursal, "
						+ "ContactoSucursal, TelefonoSucursal, CorreoElectronico "
						+ "FROM sucursal ORDER BY IdSucursal");
...
// Normalizamos: NULL → "", y dejamos '' como está
				String nomSuc = rs.getString("NombreSucursal");
				if (nomSuc == null) {
					nomSuc = "";
				}
...
// Búsqueda normalizada (NULL y '' se consideran iguales)
				buscPS.setString(1, nomSuc); // ya es "" si era null o vacío
...
// Determinamos el ID a usar (evitar colisión de PK)
				int idUsar = idAcc;
...
// === AQUÍ EL CAMBIO PRINCIPAL QUE PEDISTE ===
// Si es vacío (NULL original o ""), insertamos cadena vacía ''
	...
	private Map<Integer, Integer> mergeTecnicosAUsuario(Connection stag, Connection dest) throws SQLException {
	    ...
	    String insertUsr = "INSERT INTO usuario (idUsuario, idRol, dni, nombre, apellido, telefono, email, login, pass) VALUES(?, 2, ?, ?, ?, '', ?, ?, '1234')";
	        ...
	        while (rs.next()) {
	            ...
	            try (ResultSet er = buscNombre.executeQuery()) {
	                if (er.next()) {
	                    ...
	                    log("INFO", "Técnico " + idTec + " [" + nombreCompleto + "] encontrado por NOMBRE+APELLIDO → idUsuario=" + idDest);
	                }
	            }
	            ...
	            if (idDest == -1) {
	                ...
	                // DNI: si está vacío → poner 12345678
	                int dni = 12345678;

	                insUsr.setInt(1, idUsar);
	                insUsr.setInt(2, dni);           // dni
	                insUsr.setString(3, nombre);     // nombre
	                insUsr.setString(4, apellido);   // apellido
	                insUsr.setString(5, correo);     // email
	                insUsr.setString(6, login);      // login

	                insUsr.executeUpdate();

	                idDest = idUsar;

	                String emailLog = correo.isEmpty() ? "(sin email)" : correo;
	                log("OK", "Técnico " + idTec + " [" + nombreCompleto + "] CREADO → idUsuario=" 
	                        + idDest + " | dni=12345678 | login='" + login + "' | email='" + emailLog + "'");
	            } else {
	...
	// Auxiliar (mantenelo si ya lo tenías)
	...
	// Coincidencia: NumeroDeSerie + idCliente; fallback:
	// Nombre+Modelo+Marca+idCliente.
		...
		try (Statement st = stag.createStatement();
			...
			while (rs.next()) {
				...
				try {
					...
					insPS.setObject(10, idSucMySQL); // NULL si sucursal no existe
					insPS.executeUpdate();
					mapa.put(idAcc, idUsar);
					ins_++;
				} catch (SQLException ex) {
	...
	private String[] splitNombre(String nombreCompleto) {
	    ...
	    if (lastSpace == -1) {
	        return new String[]{trimmed, ""};           // solo nombre
	    }
	    
	    String nombre = trimmed.substring(0, lastSpace).trim();
	    String apellido = trimmed.substring(lastSpace + 1).trim();
