 ...
 *
 * CORRECCIÓN CRÍTICA:
 * - Duplicate entry '162' / '163' ... for key 'cliente.PRIMARY'
 *   → Ahora se usa un contador LOCAL nextIdLibre calculado UNA SOLA VEZ al inicio.
 *   → Se incrementa inmediatamente después de asignar → imposible colisión.
 *   → Funciona aunque haya datos residuales o migraciones anteriores fallidas.
 *
 * Además:
 * - UbicacionRemitos sigue con fuzzy name-first (MDP → id correcto en ordenesbrc).
 * - Clientes fuzzy mejorado (con y sin paréntesis).
 */
...
public class MigracionController {
    ...
    public MigracionController(ConfigMigracion config,
    ...
    public void migrarAccessAStaging() throws Exception {
        ...
        try (Connection acc  = DriverManager.getConnection(config.getUrlAccess());
             Connection stag = DriverManager.getConnection(config.getUrlStaging(),
                     config.getStagingUser(), config.getStagingPass())) {
    ...
    public void mergeStagingADestino() throws Exception {
        ...
        try (Connection stag = DriverManager.getConnection(config.getUrlStaging(),
                     config.getStagingUser(), config.getStagingPass());
             Connection dest = DriverManager.getConnection(config.getUrlDestino(),
                     config.getDestinoUser(), config.getDestinoPass())) {
    ...
    private void extClientes(Connection acc, Connection stag) throws SQLException {
        ...
        try (Statement st = acc.createStatement();
             ResultSet rs = st.executeQuery(
                     "SELECT Id,Razon_Social,CUIT,Domicilio,TelefonoEmpresa,Contacto," +
    ...
    private void extSucursales(Connection acc, Connection stag) throws SQLException {
        ...
        try (Statement st = acc.createStatement();
             ResultSet rs = st.executeQuery(
                     "SELECT IdSucursal,NombreSucursal,idClientesuc,DomicilioSucursal," +
                     "ContactoSucursal,TelefonoSucursal,CorreoElectronico FROM Sucursal ORDER BY IdSucursal");
             PreparedStatement ps = stag.prepareStatement(
                     "INSERT IGNORE INTO sucursal " +
                     "(IdSucursal,NombreSucursal,idCliente,DomicilioSucursal," +
    ...
    private void extTecnicos(Connection acc, Connection stag) throws SQLException {
        ...
        try (Statement st = acc.createStatement();
            ...
            while (rs.next()) {
                ...
                if (nombre == null || "-".equals(nombre)) nombre = "(sin asignar)";
    ...
    private void extEquipos(Connection acc, Connection stag) throws SQLException {
        ...
        try (Statement st = acc.createStatement();
             ...
             PreparedStatement ps = stag.prepareStatement(
                     ...
                     "ClienteCliente,RemitoCliente,idCliente,IdSucursal) " +
                     "VALUES(?,?,?,?,?,NULL,?,?,?,?,?)")) {
            while (rs.next()) {
        ...
        try (Statement st = stag.createStatement();
            ...
            while (rs.next()) {
                ...
                if (idMySQL != null) {
                    ...
                    omit_++;
                    if (idAcc != idMySQL) log("INFO", "  UbicRemito '" + ubic + "' id=" + idAcc + " → MySQL=" + idMySQL);
                    continue;
                }
    ...
    private Map<Integer, Integer> mergeClientes(Connection stag, Connection dest) throws SQLException {
        ...
        try (Statement st = stag.createStatement();
             ResultSet rs = st.executeQuery(
                     "SELECT idCliente,nombre,CUIT,Domicilio,TelefonoEmpresa,Contacto," +
            ...
            while (rs.next()) {
                ...
                if (idMySQLObj == null) idMySQLObj = mysqlClientesPorNombreNorm.get(nomNormFull);
    ...
    // Los métodos restantes (mergeSucursales, mergeEquipos, mergeTecnicosAUsuario, etc.)
    // se mantienen exactamente iguales a la versión anterior (usan obtenerIdLibre donde corresponda).
    ...
    private Map<Integer, Integer> mergeSucursales(Connection stag, Connection dest,
            Map<Integer, Integer> mapCliente) throws SQLException {
        ...
        String ins    = "INSERT INTO sucursal " +
                        "(IdSucursal,NombreSucursal,idCliente,DomicilioSucursal," +
                        "ContactoSucursal,TelefonoSucursal,CorreoElectronico) VALUES(?,?,?,?,?,?,?)";
        ...
        try (Statement st = stag.createStatement();
             ResultSet rs = st.executeQuery(
                     "SELECT IdSucursal,NombreSucursal,idCliente,DomicilioSucursal," +
                     "ContactoSucursal,TelefonoSucursal,CorreoElectronico FROM sucursal ORDER BY IdSucursal");
             PreparedStatement buscPS = dest.prepareStatement(buscar);
             PreparedStatement insPS  = dest.prepareStatement(ins);
             PreparedStatement updStag = stag.prepareStatement("UPDATE equipos SET IdSucursal=? WHERE IdSucursal=?")) {

            while (rs.next()) {
                int    idAcc   = rs.getInt("IdSucursal");
                String nomSuc  = rs.getString("NombreSucursal");
                if (nomSuc == null) nomSuc = "";
    ...
    // (El resto de métodos mergeEquipos, mergeTecnicosAUsuario, mergeReparaciones, mergeReemplazos, etc.
    // permanecen exactamente iguales a la versión anterior. Solo cliente fue modificado.)

    private Map<Integer, Integer> mergeEquipos(Connection stag, Connection dest,
            Map<Integer, Integer> mapCliente, Map<Integer, Integer> mapSucursal) throws SQLException {
        // ... código idéntico al anterior (sin cambios)
        Map<Integer, Integer> mapa = new HashMap<>();
        ...
        try (Statement st = stag.createStatement();
             ...
             PreparedStatement insPS     = dest.prepareStatement(ins);
             PreparedStatement updStag   = stag.prepareStatement("UPDATE reparaciones SET idEquipo=? WHERE idEquipo=?")) {
            ...
            while (rs.next()) {
                ...
                if (serie != null && !serie.trim().isEmpty() && !"null".equals(serie.trim())) {
                    ...
                    try (ResultSet er = buscSerie.executeQuery()) {
                        if (er.next()) idMySQL = er.getInt("IdEquipo");
                    }
                }
        ...
        try (Statement st = stag.createStatement();
            ...
            while (rs.next()) {
                ...
                try (ResultSet er = buscNombre.executeQuery()) {
                    if (er.next()) idDest = er.getInt("idUsuario");
                }
    ...
    private void mergeReparaciones(Connection stag, Connection dest,
            Map<Integer, Integer> mapTecnico, Map<Integer, Integer> mapEquipo,
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    try (ResultSet cr = chkDup.executeQuery()) {
                        cr.next();
                        if (cr.getInt(1) > 0) { omit_++; continue; }
                    }
                    insPS.setInt(1, els);
    ...
    private Map<Integer, Integer> mergeRemitos(Connection stag, Connection dest,
            Map<Integer, Integer> mapUbic) throws SQLException {
        // ... código idéntico al anterior (sin cambios)
        Map<Integer, Integer> mapa = new HashMap<>();
    ...
    private void verificarOCrearUsuarioBase(Connection dest, int idUsuario, String nombre,
            String apellido, String email, String login) throws SQLException {
    ...
    private String[] splitNombre(String nombreCompleto) {
        ...
        if (lastSpace == -1) return new String[]{trimmed, ""};
    ...
    private void imprimirResumen(String tipo) {
        log("INFO", "══════════════════════════════════════════════════════");
        log("OK", " RESULTADO — " + tipo + ": +" + totalInsertados + " insertados | "
                + totalOmitidos + " omitidos | " + totalErrores + " errores");
        log("INFO", "══════════════════════════════════════════════════════");
        resetContadores();
    }
