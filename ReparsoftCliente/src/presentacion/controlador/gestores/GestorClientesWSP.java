...
public class GestorClientesWSP implements MouseListener {
    ...
    // =====================================================================
    // VENTANA GESTIÓN DE CLIENTES WSP
    // =====================================================================
    ...
    private void abrirVentanaClientesWSP() {
        ...
        ventanaClientesWSP.getBtnAgregarCliente().addActionListener(e  -> modoAgregarCliente());
        ventanaClientesWSP.getBtnEditarCliente().addActionListener(e   -> modoEditarCliente());
        ventanaClientesWSP.getBtnEliminarCliente().addActionListener(e -> eliminarClienteWSP());
        ventanaClientesWSP.getBtnGuardarNuevo().addActionListener(e    -> guardarNuevoCliente());
        ventanaClientesWSP.getBtnCancelarNuevo().addActionListener(e   -> cancelarOperacion());
        ventanaClientesWSP.getBtnGuardarEdicion().addActionListener(e  -> guardarEdicionCliente());
        ventanaClientesWSP.getBtnCancelarEdicion().addActionListener(e -> cancelarOperacion());
    ...
    private void guardarNuevoCliente() {
        ...
        agenda.agregarClienteWSP(new ClienteWSPDTO(0, organizacion, nombre, telefono));
        llenarTablaClientesWSP();
        cancelarOperacion();
    ...
    private void guardarEdicionCliente() {
        if (clienteWSPSeleccionado == null) return;
    ...
    private void llenarComboOrganizacion() {
        ...
        ventanaWSP.getComboOrganizacion().addActionListener(e -> {
            ...
            if (selected != null) {
                agenda.ListarContactoxOrganizacion(
                    ventanaWSP.getComboNombreBuscado(), selected.toString());
            }
        });
    }
    ...
    private void llenarComboNombres() {
        ventanaWSP.getComboNombreBuscado().addActionListener(e -> {
            ...
            if (selected != null) {
                String telefono = agenda.obtenerTelefonoxContacto(selected.toString());
    ...
    private boolean existeClienteWSP(String telefono) {
        if (clientesWSPEnTabla == null || clientesWSPEnTabla.isEmpty()) return false;
        for (ClienteWSPDTO cliente : clientesWSPEnTabla) {
            if (cliente.getTelefonoWSP().equals(telefono)) return true;
        }
        return false;
    }
